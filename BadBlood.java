...
public class BadBlood extends JPanel implements Runnable, Constants{
	...
	BufferedImage myPicture,myPicture1, castle;
	/*SPRITE*/
	...
	public void run(){
		while(true){
			...
			if (!connected && serverData.startsWith("CONNECTED")){
				...
				center_panel.add(nameField,cons);
				...
				//Icon archer_icon = new ImageIcon("");
	...
	class CustomMouseListener implements MouseListener{
		...
		public void mouseClicked(MouseEvent e) {
			...
			if(selectedTroop=="a" && archerCount>0){
				send("DEPLOY "+userID+" "+selectedTroop+" "+e.getX()+" "+e.getY());
				archerCount--;
				if(archerCount==0){
